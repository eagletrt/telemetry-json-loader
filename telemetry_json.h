...
typedef struct damper_rear_o{
    double bound;
    double rebound;
    double preload;
}damper_rear_o;

typedef struct damper_front_o{
    double bound;
    double rebound;
    double preload;
}damper_front_o;
...
typedef struct telemetry_config_t{
    ...
    bool connection_downsample_skip_same_data;
...
typedef struct car_config_t{
    ...
    damper_front_o damper_front;
    damper_rear_o damper_rear;
    pilot_o pilot;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("track_conditions_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("pilot_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
template <>
bool CheckJson(const damper_rear_o& obj, const rapidjson::Document& doc)
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("damper_rear_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
    if(!doc.HasMember("bound")){
        JSON_LOG_FUNC("damper_rear_o MISSING FIELD: bound"); 
        check = false;
    }
    if(!doc.HasMember("rebound")){
        JSON_LOG_FUNC("damper_rear_o MISSING FIELD: rebound"); 
        check = false;
    }
    if(!doc.HasMember("preload")){
        JSON_LOG_FUNC("damper_rear_o MISSING FIELD: preload"); 
        check = false;
    }
    return check;
}
...
template<>
void Serialize(rapidjson::Value& out, const damper_rear_o& obj, rapidjson::Document::AllocatorType& alloc)
{
    out.SetObject();
...
template<>
void Deserialize(damper_rear_o& obj, rapidjson::Value& doc)
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("damper_rear_o NOT AN OBJECT");
        return;
...
template <>
bool CheckJson(const damper_front_o& obj, const rapidjson::Document& doc)
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("damper_front_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
    if(!doc.HasMember("bound")){
        JSON_LOG_FUNC("damper_front_o MISSING FIELD: bound"); 
        check = false;
    }
    if(!doc.HasMember("rebound")){
        JSON_LOG_FUNC("damper_front_o MISSING FIELD: rebound"); 
        check = false;
    }
    if(!doc.HasMember("preload")){
        JSON_LOG_FUNC("damper_front_o MISSING FIELD: preload"); 
        check = false;
    }
    return check;
}
...
template<>
void Serialize(rapidjson::Value& out, const damper_front_o& obj, rapidjson::Document::AllocatorType& alloc)
{
    out.SetObject();
    out.AddMember("bound", rapidjson::Value().SetDouble(obj.bound), alloc);
    out.AddMember("rebound", rapidjson::Value().SetDouble(obj.rebound), alloc);
...
template<>
void Deserialize(damper_front_o& obj, rapidjson::Value& doc)
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("damper_front_o NOT AN OBJECT");
        return;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("wheel_rear_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("wheel_front_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
    if(!doc.HasMember("camber")){
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("aero_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
    ...
    if(!doc.HasMember("flap")){
        JSON_LOG_FUNC("aero_o MISSING FIELD: flap"); 
        check = false;
    }
    return check;
}
...
void Serialize(rapidjson::Value& out, const aero_o& obj, rapidjson::Document::AllocatorType& alloc)
{
    ...
    out.AddMember("flap", rapidjson::Value().SetString(obj.flap.c_str(), obj.flap.size(), alloc), alloc);
}
template<>
void Deserialize(aero_o& obj, rapidjson::Value& doc)
{
    ...
    if(!doc.HasMember("flap") || !doc["flap"].IsString()){
        JSON_LOG_FUNC("aero_o MISSING FIELD: flap"); 
    }else{
        obj.flap = std::string(doc["flap"].GetString(), doc["flap"].GetStringLength());
    }
}
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("connection_o NOT AN OBJECT");
        return false;
    }
    bool check = true;
    if(!doc.HasMember("ip")){
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("gps_devices_a NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("can_devices_a NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("telemetry_config_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
    ...
    if(!doc.HasMember("connection_downsample_skip_same_data")){
        JSON_LOG_FUNC("telemetry_config_t MISSING FIELD: connection_downsample_skip_same_data"); 
        check = false;
    }
...
void Serialize(rapidjson::Document& out, const telemetry_config_t& obj)
{
    ...
    out.AddMember("connection_downsample_skip_same_data", rapidjson::Value().SetBool(obj.connection_downsample_skip_same_data), alloc);
...
void Deserialize(telemetry_config_t& obj, rapidjson::Document& doc)
{
    ...
    if(!doc.HasMember("connection_downsample_skip_same_data") || !doc["connection_downsample_skip_same_data"].IsBool()){
        JSON_LOG_FUNC("telemetry_config_t MISSING FIELD: connection_downsample_skip_same_data"); 
    }else{
        obj.connection_downsample_skip_same_data = doc["connection_downsample_skip_same_data"].GetBool();
    }
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("car_config_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
    ...
    if(!doc.HasMember("damper_front")){
        JSON_LOG_FUNC("car_config_t MISSING FIELD: damper_front"); 
        check = false;
    }
    if(!doc.HasMember("damper_rear")){
        JSON_LOG_FUNC("car_config_t MISSING FIELD: damper_rear"); 
        check = false;
    }
...
void Serialize(rapidjson::Document& out, const car_config_t& obj)
{
    ...
    {
        rapidjson::Value v;
        Serialize(v, obj.damper_front, alloc);
        out.AddMember("damper_front", v, alloc);
    }
    {
        rapidjson::Value v;
        Serialize(v, obj.damper_rear, alloc);
        out.AddMember("damper_rear", v, alloc);
    }
    {
        rapidjson::Value v;
...
void Deserialize(car_config_t& obj, rapidjson::Document& doc)
{
    ...
    if(!doc.HasMember("damper_front") || !doc["damper_front"].IsObject()){
        JSON_LOG_FUNC("car_config_t MISSING FIELD: damper_front"); 
    }else{
        Deserialize(obj.damper_front, doc["damper_front"]);
    }
    if(!doc.HasMember("damper_rear") || !doc["damper_rear"].IsObject()){
        JSON_LOG_FUNC("car_config_t MISSING FIELD: damper_rear"); 
    }else{
        Deserialize(obj.damper_rear, doc["damper_rear"]);
    }
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("session_config_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("telemetry_login_data_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("stat_json_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
...
{
    if(!doc.IsObject()){
        JSON_LOG_FUNC("csv_parser_config_t NOT AN OBJECT");
        return false;
    }
    bool check = true;
